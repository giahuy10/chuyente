 ...
 * @since  1.6
 */
class RateModelDates extends JModelList
{
/**
	* Constructor.
	*
	...
	*/
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Filtering time
		$this->setState('filter.time', $app->getUserStateFromRequest($this->context.'.filter.time', 'filter_time', '', 'string'));


		// Load the parameters.
		$params = JComponentHelper::getParams('com_rate');
	...
	protected function getListQuery()
	{
		...
		if (!empty($search))
		{
			...
			else
			{
				...
				$query->where('( a.date LIKE ' . $search . '  OR  a.buy_rate LIKE ' . $search . '  OR  a.offer_rate LIKE ' . $search . '  OR  a.time LIKE ' . $search . ' )');
			}
		}
		...
		//Filtering time
		$filter_time = $this->state->get("filter.time");
		if ($filter_time !== null && !empty($filter_time))
		{
			$query->where("a.`time` = '".$db->escape($filter_time)."'");
		}
	...
	public function getItems()
	{
		...
		foreach ($items as $oneItem) {
					$oneItem->time = JText::_('COM_RATE_DATES_TIME_OPTION_' . strtoupper($oneItem->time));
		}
		return $items;
	}
}
